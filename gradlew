...
#       installation, make sure it is on your PATH.
#
##############################################################################
...
#   * Put all other arguments, including class names, after the class name.
set -- \
...
if ! "$cygwin" && ! "$msys" && ! "$nonstop" ; then
    case $( set -- $(LANG=C LC_ALL=C locale charmap 2>/dev/null) && echo $1 ) in #(
      UTF-8) ;;
      *)
        if [ -t 1 ] && command -v stty > /dev/null; then
            stty_saved=$(stty -g)
